...
// 1. Cut off the duplicate Phase 9 blocks
...
// 2. Remove any fields we previously tried to inject from Tenant, Vendor, Invoice
...
lines = lines.filter(line => {
  ...
  if (insideModel && (currentModel === 'Tenant' || currentModel === 'Vendor' || currentModel === 'Invoice')) {
    const propName = line.trim().split(/\\s+/)[0];
...
// Rejoin the cleaned schema
schema = lines.join('\\n');

// 3. Inject the relations properly
...
console.log('Final Schema built successfully.');
