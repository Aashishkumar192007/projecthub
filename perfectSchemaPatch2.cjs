...
// 1. Remove Phase 9 marker and everything after it to avoid duplicating Phase 9 models
...
// 2. Clean duplicate fields inside every model
...
for (let i = 0; i < lines.length; i++) {
  ...
  if (insideModel && trimmed.length > 0 && !trimmed.startsWith('@@') && !trimmed.startsWith('//')) {
    ...
    if (currentModelFields.has(propName)) {
      // Duplicate field! Skip it.
      continue;
    } else {
      currentModelFields.add(propName);
      outputLines.push(line);
    }
  } else {
    // Other lines (empty, comments, block attributes)
    outputLines.push(line);
  }
}

// Ensure Tenant, Vendor, Invoice have their Phase 9 relations
let finalSchemaStr = outputLines.join('\\n');

function ensureRelation(modelName, relationsStr) {
  const regex = new RegExp('model ' + modelName + ' \\{[\\\\s\\\\S]*?\\n\\}');
  finalSchemaStr = finalSchemaStr.replace(regex, match => {
    return match.replace(/\\n\\}$/, '\\n' + relationsStr + '\\n}');
  });
}

ensureRelation('Tenant', [
...
ensureRelation('Vendor', [
  '  kyc                     VendorKyc?',
  '  performance             VendorPerformance?',
  '  purchaseOrders          PurchaseOrder[]'
].join('\\n'));

ensureRelation('Invoice', [
...
fs.writeFileSync(schemaPath, finalSchemaStr + '\\n' + newModels);
console.log('Schema fixed and completely clean!');
