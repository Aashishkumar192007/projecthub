...
schema = schema.replace(/(model Tenant {[\s\S]*?)(?=})/, `$1${tenantAdditions}`);
schema = schema.replace(/(model Vendor {[\s\S]*?)(?=})/, `$1${vendorAdditions}`);
schema = schema.replace(/(model Invoice {[\s\S]*?)(?=})/, `$1${invoiceAdditions}`);
...
console.log('Phase 9 Schema patched successfully.');
