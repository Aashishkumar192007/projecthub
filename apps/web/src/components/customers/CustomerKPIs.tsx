...
function KPICard({ label, value, sub, icon: Icon, color, borderColor }: KPICardProps) {
  return (
    <div className={`flex-1 min-w-0 bg-[#161616] border ${borderColor} rounded-xl p-4 flex items-start gap-3 group hover:bg-[#1A1A1A] transition-colors`}>
  ...
  return (
    <div className="flex gap-3 px-6 py-4 border-b border-[#2A2A30]/50 overflow-x-auto no-scrollbar shrink-0">
      <KPICard
        ...
        sub={`${kpis.newThisMonth} new this month`}
        icon={Users}
        color="bg-[#93A5CF]/15 text-[#93A5CF]"
        borderColor="border-[#93A5CF]/20"
      />
      <KPICard
        label="Owners"
        value={String(kpis.totalOwners)}
        sub="Properties & investors"
        icon={Home}
      ...
      <KPICard
        label="Residents"
        value={String(kpis.totalResidents)}
        sub="Tenants & residents"
        icon={User}
      ...
      <KPICard
        label="Corporate"
        value={String(kpis.totalCorporate)}
      ...
      <KPICard
        ...
        sub={`${kpis.delinquentCount} delinquent accounts`}
        icon={CreditCard}
      ...
      <KPICard
        label="VIP Accounts"
        value={String(kpis.vipCount)}
        sub="Priority relationships"
        icon={Star}
