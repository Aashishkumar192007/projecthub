...
export function ProcurementCopilot() {
  const { entities, activeCategoryId } = useProcurementStore();
  ...
  return (
    <div className="w-80 border-l border-[#2A2A30] bg-[#111111] h-full flex flex-col shrink-0 overflow-y-auto z-10">
      ...
      <div className="p-5 border-b border-[#2A2A30] bg-[#161616] flex items-center gap-3 sticky top-0 z-10">
        ...
        <div>
          <h2 className="text-sm font-bold text-white flex items-center gap-2">Supply Chain AI <Sparkles size={12} className="text-[#00E5FF]"/></h2>
          <p className="text-[10px] text-[#A1A1AA] uppercase tracking-wider font-bold">Procurement Analyst</p>
        </div>
      </div>
      ...
      <div className="p-5 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-[#1A1A1A] border border-[#2A2A30] rounded-lg p-3">
            <p className="text-[9px] text-[#A1A1AA] font-bold uppercase tracking-widest mb-1 flex items-center gap-1">
              <DollarSign size={10} className="text-[#00E5FF]"/> Pipeline Val
            </p>
            <p className="text-sm font-black text-white mt-2">
              ₹{(activeEntities.reduce((acc, curr) => acc + (curr.value || 0), 0) / 100000).toFixed(1)}L
            </p>
          </div>
          <div className="bg-[#1A1A1A] border border-[#2A2A30] rounded-lg p-3">
            <p className="text-[9px] text-[#A1A1AA] font-bold uppercase tracking-widest mb-1 flex items-center gap-1">
        ...
        <div>
          <p className="text-[10px] font-bold text-[#A1A1AA] tracking-widest uppercase mb-3 flex items-center gap-2">
            <Bot size={12} className="text-[#00E5FF]" /> Supply Chain Intelligence
          </p>
          <div className="space-y-3">
            {activeEntities.length > 0 && activeEntities.some(e => e.insights.length > 0) ? (
              ...
              <div className="bg-[#1A1A1A] border border-[#2A2A30] rounded-xl p-4 text-center">
                ...
                <p className="text-xs font-bold text-success">Supply Chain is operating nominally.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="pt-4 border-t border-[#2A2A30]">
           ...
           <div className="grid grid-cols-2 gap-2">
             <button className="p-3 bg-[#1A1A1A] hover:bg-[#1E1E22] border border-[#2A2A30] rounded-lg text-left transition-colors">
               <p className="text-xs font-bold text-white">Create PO</p>
               <p className="text-[9px] text-[#71717A] mt-1">From Requisition</p>
             </button>
             <button className="p-3 bg-[#1A1A1A] hover:bg-[#1E1E22] border border-[#2A2A30] rounded-lg text-left transition-colors">
               <p className="text-xs font-bold text-white">Send RFQ</p>
               <p className="text-[9px] text-[#71717A] mt-1">To Top Vendors</p>
             </button>
             <button className="p-3 bg-[#1A1A1A] hover:bg-[#1E1E22] border border-[#2A2A30] rounded-lg text-left transition-colors">
               <p className="text-xs font-bold text-white">GRN Log</p>
               <p className="text-[9px] text-[#71717A] mt-1">Receive Goods</p>
             </button>
             <button className="p-3 bg-[#1A1A1A] hover:bg-[#1E1E22] border border-[#2A2A30] rounded-lg text-left transition-colors">
               <p className="text-xs font-bold text-white">Alert</p>
               <p className="text-[9px] text-[#71717A] mt-1">Notify Dept Head</p>
             </button>
           </div>
        </div>
