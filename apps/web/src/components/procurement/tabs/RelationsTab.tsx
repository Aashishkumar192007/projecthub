...
import { Layers, Package, ShoppingCart, Users, Target, Warehouse, FileSignature } from 'lucide-react';

export function RelationsTab() {
  const { activeCategoryId } = useProcurementStore();
      ...
      <div className="relative w-full h-[600px] bg-[#0A0C10] overflow-hidden flex items-center justify-center rounded-xl border border-[#2A2A30]">
        ...
        <div className="relative w-[700px] h-[500px]">
          ...
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-30 flex flex-col items-center gap-2 cursor-pointer hover:scale-105 transition-transform">
            <div className={`w-20 h-20 bg-[#1A1A1A] rounded-full flex items-center justify-center shadow-[0_0_40px_rgba(0,229,255,0.2)] border-2 border-[#00E5FF]`}>
            ...
            <span className="text-[10px] font-bold text-[#00E5FF] uppercase tracking-widest bg-[#00E5FF]/10 px-2 py-0.5 rounded border border-[#00E5FF]/30">SCM Category</span>
          </div>

          {/* Orbiting ERP Nodes */}
          {renderNode('pos', `84 Open POs`, <ShoppingCart size={18} className="text-[#00E5FF]" />, 'text-[#00E5FF]', 6)}
          {renderNode('vendors', `342 Vendors`, <Users size={18} className="text-brand-blue" />, 'text-brand-blue', 6)}
          {renderNode('inventory', `₹42M Inventory`, <Package size={18} className="text-warning" />, 'text-warning', 6)}
          {renderNode('warehouse', `14 Warehouses`, <Warehouse size={18} className="text-[#A1A1AA]" />, 'text-[#A1A1AA]', 6)}
          {renderNode('contract', `840 Contracts`, <FileSignature size={18} className="text-success" />, 'text-success', 6)}
          {renderNode('budget', `68% Consumed`, <Target size={18} className="text-danger" />, 'text-danger', 6)}
