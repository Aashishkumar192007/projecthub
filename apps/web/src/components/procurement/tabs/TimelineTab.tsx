...
import { Clock, ShoppingCart, PackageCheck, FileText, Package, Users, AlertTriangle } from 'lucide-react';

export function TimelineTab() {
  const { events, activeCategoryId } = useProcurementStore();
  const activeEvents = events.filter(e => e.categoryId === activeCategoryId);
  
  const getEventIcon = (type: string) => {
    switch (type) {
      case 'PR Created': return <FileText size={14} className="text-brand-blue" />;
      case 'PO Approved': return <ShoppingCart size={14} className="text-[#00E5FF]" />;
      case 'Goods Received': return <PackageCheck size={14} className="text-success" />;
      case 'Vendor Rated': return <Users size={14} className="text-purple-400" />;
      case 'Stock Alert': return <AlertTriangle size={14} className="text-danger" />;
    ...
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between mb-2">
        <h3 className="text-sm font-bold text-white flex items-center gap-2">
          <Clock size={16} className="text-[#00E5FF]"/> Supply Chain Activity Feed
        </h3>
      </div>
      ...
      {activeEvents.length > 0 ? (
        ...
        <div className="p-12 text-center border border-dashed border-[#2A2A30] rounded-xl bg-[#1A1A1A]">
          ...
          <p className="text-xs text-[#71717A]">PRs, POs, and Receipts will appear here.</p>
        </div>
      )}
    </div>
  );
