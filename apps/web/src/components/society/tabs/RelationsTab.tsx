...
import { Home, UserSquare2, Users, Headset, Map, Calendar, Shield } from 'lucide-react';

export function RelationsTab() {
  const { activeCategoryId } = useSocietyStore();
      ...
      <div className="relative w-full h-[600px] bg-[#0A0C10] overflow-hidden flex items-center justify-center rounded-xl border border-[#2A2A30]">
        ...
        <div className="relative w-[700px] h-[500px]">
          ...
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-30 flex flex-col items-center gap-2 cursor-pointer hover:scale-105 transition-transform">
            ...
            <span className="text-[10px] font-bold text-[#00E5FF] uppercase tracking-widest bg-[#00E5FF]/10 px-2 py-0.5 rounded border border-[#00E5FF]/30">Governance Node</span>
          </div>

          {/* Orbiting Community Nodes */}
          {renderNode('residents', `1,420 Residents`, <UserSquare2 size={18} className="text-success" />, 'text-success', 6)}
          {renderNode('committee', `12 Members`, <Users size={18} className="text-purple-400" />, 'text-purple-400', 6)}
          {renderNode('facilities', `8 Amenities`, <Map size={18} className="text-[#00E5FF]" />, 'text-[#00E5FF]', 6)}
          {renderNode('events', `3 Upcoming`, <Calendar size={18} className="text-warning" />, 'text-warning', 6)}
          {renderNode('complaints', `14 Open Tickets`, <Headset size={18} className="text-danger" />, 'text-danger', 6)}
          {renderNode('staff', `84 Staff`, <Shield size={18} className="text-[#A1A1AA]" />, 'text-[#A1A1AA]', 6)}
