...
import { Clock, Headset, Megaphone, PieChart, UserPlus, Calendar, IndianRupee } from 'lucide-react';
...
export function TimelineTab() {
  ...
  const getEventIcon = (type: string) => {
    switch (type) {
      case 'Complaint Raised': return <Headset size={14} className="text-danger" />;
      case 'Notice Published': return <Megaphone size={14} className="text-brand-blue" />;
      case 'Poll Opened': return <PieChart size={14} className="text-purple-400" />;
      case 'Visitor Approved': return <UserPlus size={14} className="text-[#A1A1AA]" />;
      case 'Event Scheduled': return <Calendar size={14} className="text-warning" />;
      case 'Payment Received': return <IndianRupee size={14} className="text-success" />;
    ...
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between mb-2">
        <h3 className="text-sm font-bold text-white flex items-center gap-2">
          <Clock size={16} className="text-[#00E5FF]"/> Community Activity Feed
        </h3>
      </div>
      ...
      {activeEvents.length > 0 ? (
        ...
        <div className="p-12 text-center border border-dashed border-[#2A2A30] rounded-xl bg-[#1A1A1A]">
          ...
          <p className="text-xs text-[#71717A]">Complaints, notices, and events will appear here.</p>
        </div>
      )}
    </div>
  );
