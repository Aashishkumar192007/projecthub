...
import { Bot, Sparkles, Activity, ShieldAlert, CheckCircle2, HeartPulse, ArrowRight } from 'lucide-react';

export function SocietyCopilot() {
  ...
  return (
    <div className="w-80 border-l border-[#2A2A30] bg-[#111111] h-full flex flex-col shrink-0 overflow-y-auto z-10">
      ...
      <div className="p-5 border-b border-[#2A2A30] bg-[#161616] flex items-center gap-3 sticky top-0 z-10">
        ...
        <div>
          <h2 className="text-sm font-bold text-white flex items-center gap-2">Community AI <Sparkles size={12} className="text-[#00E5FF]"/></h2>
          <p className="text-[10px] text-[#A1A1AA] uppercase tracking-wider font-bold">Governance Copilot</p>
        </div>
      </div>
      ...
      <div className="p-5 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-[#1A1A1A] border border-[#2A2A30] rounded-lg p-3">
            <p className="text-[9px] text-[#A1A1AA] font-bold uppercase tracking-widest mb-1 flex items-center gap-1">
              <HeartPulse size={10} className="text-[#00E5FF]"/> Sentiments
            </p>
          ...
          <div className="bg-[#1A1A1A] border border-[#2A2A30] rounded-lg p-3">
            <p className="text-[9px] text-[#A1A1AA] font-bold uppercase tracking-widest mb-1 flex items-center gap-1">
              <Activity size={10} className="text-brand-blue"/> Active Trackers
            </p>
            <p className="text-2xl font-black text-white">{activeEntities.length}</p>
          </div>
        </div>
        ...
        <div>
          <p className="text-[10px] font-bold text-[#A1A1AA] tracking-widest uppercase mb-3 flex items-center gap-2">
            <Bot size={12} className="text-[#00E5FF]" /> Governance Intelligence
          </p>
          <div className="space-y-3">
            {activeEntities.length > 0 && activeEntities.some(e => e.insights.length > 0) ? (
              activeEntities.flatMap(entity => entity.insights.map((insight) => (
                <div key={insight.id} className={`bg-[#1A1A1A] border rounded-xl p-4 relative overflow-hidden group ${
                  ...
                  <div className="flex justify-between items-start mb-2">
                     <span className={`text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded border ${
                       insight.type === 'Recommendation' ? 'bg-brand-blue/10 text-brand-blue border-brand-blue/30' :
              ...
              <div className="bg-[#1A1A1A] border border-[#2A2A30] rounded-xl p-4 text-center">
                ...
                <p className="text-xs font-bold text-success">Governance is operating nominally.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="pt-4 border-t border-[#2A2A30]">
           ...
           <div className="grid grid-cols-2 gap-2">
             <button className="p-3 bg-[#1A1A1A] hover:bg-[#1E1E22] border border-[#2A2A30] rounded-lg text-left transition-colors">
               ...
               <p className="text-[9px] text-[#71717A] mt-1">Send Notice</p>
             </button>
             <button className="p-3 bg-[#1A1A1A] hover:bg-[#1E1E22] border border-[#2A2A30] rounded-lg text-left transition-colors">
               <p className="text-xs font-bold text-white">Poll</p>
               <p className="text-[9px] text-[#71717A] mt-1">Gather Input</p>
             </button>
             <button className="p-3 bg-[#1A1A1A] hover:bg-[#1E1E22] border border-[#2A2A30] rounded-lg text-left transition-colors">
               <p className="text-xs font-bold text-white">Work Order</p>
               <p className="text-[9px] text-[#71717A] mt-1">Facility Repair</p>
             </button>
             <button className="p-3 bg-[#1A1A1A] hover:bg-[#1E1E22] border border-[#2A2A30] rounded-lg text-left transition-colors">
               <p className="text-xs font-bold text-white">Report</p>
               <p className="text-[9px] text-[#71717A] mt-1">Gen Minutes</p>
             </button>
           </div>
        </div>
