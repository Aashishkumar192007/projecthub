'use client';

import { useCrmStore } from '@/store/crmStore';
import { Gift, Plus, TrendingUp } from 'lucide-react';
import { toast } from 'sonner';


export function ReferralsView() {
  const { referrals } = useCrmStore();

  return (
    <div className="p-6 bg-[#0A0C10] min-h-full flex flex-col">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-medium text-white tracking-wide">Referral Program Engine</h2>
        <button onClick={(e) => { e.stopPropagation(); toast.info('Feature Coming Soon', { description: 'This action is part of the upcoming release.' }); }} className="bg-[#00E5FF] hover:bg-[#00E5FF]/80 text-[#0A1A2A] px-4 py-2 rounded text-sm font-medium transition-colors flex items-center gap-2">
          <Plus className="w-4 h-4" />
          <span>Log Referral</span>
        </button>
      </div>

      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-[#121212] border border-neutral-800 rounded p-4 border-l-2 border-l-[#00E5FF]">
          <div className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest mb-2">ACTIVE REFERRALS</div>
          <div className="text-2xl font-medium text-white">{referrals.length}</div>
        </div>
        <div className="bg-[#121212] border border-neutral-800 rounded p-4 border-l-2 border-l-green-400">
          <div className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest mb-2">CONVERSIONS</div>
          <div className="text-2xl font-medium text-white">{referrals.filter(r => r.status === 'CONVERTED').length}</div>
        </div>
        <div className="bg-[#121212] border border-neutral-800 rounded p-4 border-l-2 border-l-amber-400">
          <div className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest mb-2">COMMISSION POOL</div>
          <div className="text-2xl font-medium text-white">
            ${referrals.reduce((sum, r) => sum + (r.commissionAmount || 0), 0).toLocaleString()}
          </div>
        </div>
      </div>

      <div className="flex-1 bg-[#121212] border border-neutral-800 rounded overflow-hidden">
        <table className="w-full text-sm text-left whitespace-nowrap">
          <thead className="bg-[#1A1C20] border-b border-neutral-800">
            <tr className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest">
              <th className="px-6 py-4 font-medium">REFERRER</th>
              <th className="px-6 py-4 font-medium">TYPE</th>
              <th className="px-6 py-4 font-medium">REFERRED LEAD</th>
              <th className="px-6 py-4 font-medium text-right">COMMISSION</th>
              <th className="px-6 py-4 font-medium text-right">STATUS</th>
            </tr>
          </thead>
          <tbody>
            {referrals.map(ref => (
              <tr key={ref.id} className="border-b border-neutral-800/50 hover:bg-[#1A1C20] transition-colors">
                <td className="px-6 py-4 font-medium text-white flex items-center gap-2">
                  <Gift className="w-4 h-4 text-neutral-500" />
                  {ref.referrerId}
                </td>
                <td className="px-6 py-4 text-neutral-400">{ref.referrerType}</td>
                <td className="px-6 py-4 text-neutral-300">{ref.referredLeadId}</td>
                <td className="px-6 py-4 text-right text-white">
                  {ref.commissionAmount ? `$${ref.commissionAmount.toLocaleString()}` : '-'}
                </td>
                <td className="px-6 py-4 text-right">
                  <span className={`px-2.5 py-1 text-[10px] font-bold tracking-widest uppercase rounded border ${
                    ref.status === 'CONVERTED' ? 'bg-green-500/10 text-green-400 border-green-500/20' : 
                    ref.status === 'QUALIFIED' ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' :
                    'bg-neutral-800 text-neutral-500 border-neutral-700'
                  }`}>
                    {ref.status}
                  </span>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    </div>
  );
}
