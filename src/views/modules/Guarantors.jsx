...
const Guarantors = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await apiClient.get('/guarantors');
      setData(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await apiClient.post('/guarantors', formData);
      setShowModal(false);
      setFormData({});
      fetchData();
    } catch (err) {
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', height: '100%', position: 'relative' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ fontSize: '1.5rem', marginBottom: '0.25rem' }}>Vendor Guarantors</h1>
          <p>Formalise vendor trust chains.</p>
        </div>
