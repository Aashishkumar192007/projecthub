...
const ConstructionProjects = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await apiClient.get('/construction');
      setData(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await apiClient.post('/construction', formData);
      setShowModal(false);
      setFormData({});
      fetchData();
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await apiClient.delete('/construction/' + id);
      fetchData();
    } catch (err) {
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', height: '100%', position: 'relative' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ fontSize: '1.5rem', marginBottom: '0.25rem' }}>Construction Projects</h1>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
              <thead>
                <tr style={{ background: 'var(--bg-main)', color: 'var(--text-secondary)', fontSize: '0.75rem', textTransform: 'uppercase' }}>
                  <th style={{ padding: '1rem 1.5rem', fontWeight: 600 }}>Project Name</th>
                  <th style={{ padding: '1rem 1.5rem', fontWeight: 600 }}>Phase</th>
                  <th style={{ padding: '1rem 1.5rem', fontWeight: 600 }}>Budget</th>
                  <th style={{ padding: '1rem 1.5rem', fontWeight: 600 }}>Manager</th>
              ...
              <tbody>
                {data.map((row, idx) => (
                  <tr key={row.id} style={{ borderTop: idx > 0 ? '1px solid var(--border-color)' : 'none' }}>
                    <td style={{ padding: '1rem 1.5rem', fontSize: '0.875rem' }}>{row.name || '-'}</td>
                    <td style={{ padding: '1rem 1.5rem', fontSize: '0.875rem' }}>{row.phase || '-'}</td>
                    <td style={{ padding: '1rem 1.5rem', fontSize: '0.875rem' }}>{row.budget || '-'}</td>
                    <td style={{ padding: '1rem 1.5rem', fontSize: '0.875rem' }}>{row.manager || '-'}</td>
          ...
          <div className="card animate-fade-in" style={{ position: 'relative', width: '400px', maxWidth: '90%', zIndex: 101, display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  <label style={{ fontSize: '0.875rem', fontWeight: 500, color: 'var(--text-primary)' }}>Project Name</label>
                  <input 
                    type="text" 
                    required 
                    value={formData.name || ''}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  <label style={{ fontSize: '0.875rem', fontWeight: 500, color: 'var(--text-primary)' }}>Phase</label>
                  <input 
                    type="text" 
                    required 
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  <label style={{ fontSize: '0.875rem', fontWeight: 500, color: 'var(--text-primary)' }}>Budget</label>
                  <input 
                    type="text" 
                    required 
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  <label style={{ fontSize: '0.875rem', fontWeight: 500, color: 'var(--text-primary)' }}>Manager</label>
                  <input 
                    type="text" 
                    required 
