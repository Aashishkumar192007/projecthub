  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', height: '100%', position: 'relative' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ fontSize: '1.5rem', marginBottom: '0.25rem' }}>Employees & HR</h1>
              ...
              <tbody>
                {data.map((row, idx) => (
                  <tr key={row.id} style={{ borderTop: idx > 0 ? '1px solid var(--border-color)' : 'none' }}>
                    ...
                    <td style={{ padding: '1rem 1.5rem', fontSize: '0.875rem' }}>{row.role || '-'}</td>
                    <td style={{ padding: '1rem 1.5rem', fontSize: '0.875rem' }}>{row.department || '-'}</td>
