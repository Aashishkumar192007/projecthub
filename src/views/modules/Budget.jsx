...
const Budget = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await apiClient.post('/budget', formData);
      setShowModal(false);
      setFormData({});
      fetchData();
    } catch (err) {
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', height: '100%', position: 'relative' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ fontSize: '1.5rem', marginBottom: '0.25rem' }}>Budget</h1>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
              <thead>
                <tr style={{ background: 'var(--bg-main)', color: 'var(--text-secondary)', fontSize: '0.75rem', textTransform: 'uppercase' }}>
                  ...
                  <th style={{ padding: '1rem 1.5rem', fontWeight: 600 }}>Allocated</th>
                  <th style={{ padding: '1rem 1.5rem', fontWeight: 600 }}>Spent</th>
              ...
              <tbody>
                {data.map((row, idx) => (
                  <tr key={row.id} style={{ borderTop: idx > 0 ? '1px solid var(--border-color)' : 'none' }}>
                    <td style={{ padding: '1rem 1.5rem', fontSize: '0.875rem' }}>{row.project || '-'}</td>
                    <td style={{ padding: '1rem 1.5rem', fontSize: '0.875rem' }}>{row.allocated || '-'}</td>
                    <td style={{ padding: '1rem 1.5rem', fontSize: '0.875rem' }}>{row.spent || '-'}</td>
          ...
          <div className="card animate-fade-in" style={{ position: 'relative', width: '400px', maxWidth: '90%', zIndex: 101, display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  <label style={{ fontSize: '0.875rem', fontWeight: 500, color: 'var(--text-primary)' }}>Project</label>
                  <input 
                    type="text" 
                    required 
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  <label style={{ fontSize: '0.875rem', fontWeight: 500, color: 'var(--text-primary)' }}>Allocated</label>
                  <input 
                    type="text" 
                    required 
                    value={formData.allocated || ''}
                    onChange={(e) => handleInputChange(e, 'allocated')}
                    style={{ padding: '0.625rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-color)', outline: 'none' }}
                  />
                </div>
              
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  <label style={{ fontSize: '0.875rem', fontWeight: 500, color: 'var(--text-primary)' }}>Spent</label>
                  <input 
                    type="text" 
                    required 
